...
template<typename T>
int compareList(T first, T second)
{
    ...
    return memcmp(first.list, second.list, first.count);
}

int compareMacAddress(sai_mac_t first, sai_mac_t second)
{
    return memcmp(first, second, sizeof(sai_mac_t));
}

int compareIpV4Address(sai_ip4_t first, sai_ip4_t second)
{
    if (first == second)
    {
        return 0;
...
int compareIpV6Address(sai_ip6_t first, sai_ip6_t second)
{
    return memcmp(first, second, sizeof(sai_ip6_t));
}

int compareIpAddress(sai_ip_address_t first, sai_ip_address_t second)
{
    if (first.addr_family != second.addr_family)
    {
        if (first.addr_family < second.addr_family)
        {
            return -1;
    ...
    switch (first.addr_family)
    {
    case SAI_IP_ADDR_FAMILY_IPV4:
        return compareIpV4Address(first.addr.ip4, second.addr.ip4);
    case SAI_IP_ADDR_FAMILY_IPV6:
        return compareIpV6Address(first.addr.ip6, second.addr.ip6);
    default:
        SWSS_LOG_THROW("Unknown IP address famility %d", first.addr_family);
    }
    SWSS_LOG_THROW("BUG: Unexpectedly reached dead code block");
}
...
bool compareAclAction(sai_acl_entry_attr_t id, sai_acl_action_data_t first, sai_acl_action_data_t second)
{
    ...
    case SAI_ATTR_VALUE_TYPE_ACL_ACTION_DATA_MAC:
        return compareMacAddress(first.parameter.mac, second.parameter.mac) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_ACTION_DATA_IPV4:
        return compareIpV4Address(first.parameter.ip4, second.parameter.ip4) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_ACTION_DATA_IPV6:
        return compareIpV6Address(first.parameter.ip6, second.parameter.ip6) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_ACTION_DATA_OBJECT_ID:
        return first.parameter.oid < second.parameter.oid;
    case SAI_ATTR_VALUE_TYPE_ACL_ACTION_DATA_OBJECT_LIST:
        return compareList(first.parameter.objlist, second.parameter.objlist) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_ACTION_DATA_IP_ADDRESS:
        return compareIpAddress(first.parameter.ipaddr, second.parameter.ipaddr) < 0;
    default:
        SWSS_LOG_THROW("Unhandled ACL field type %d", meta->attrvaluetype);
    }

    SWSS_LOG_THROW("BUG: Reached unexpected code block");
}
...
bool compareAclField(sai_acl_entry_attr_t id, sai_acl_field_data_t first, sai_acl_field_data_t second)
{
    ...
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_BOOL:
        return first.data.booldata < second.data.booldata;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT8:
        if (first.mask.u8 != second.mask.u8)
        {
            return first.mask.u8 < second.mask.u8;
        }
        return first.data.u8 < second.data.u8;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_INT8:
        if (first.mask.s8 != second.mask.s8)
        {
            return first.mask.s8 < second.mask.s8;
        }
        return first.data.s8 < second.data.s8;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT16:
        if (first.mask.u16 != second.mask.u16)
        {
            return first.mask.u16 < second.mask.u16;
        }
        return first.data.u16 < second.data.u16;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_INT16:
        if (first.mask.s16 != second.mask.s16)
        {
            return first.mask.s16 < second.mask.s16;
        }
        return first.data.s16 < second.data.s16;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT32:
        if (first.mask.u32 != second.mask.u32)
        {
            return first.mask.u32 < second.mask.u32;
        }
        return first.data.u32 < second.data.u32;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_INT32:
        if (first.mask.s32 != second.mask.s32)
        {
            return first.mask.s32 < second.mask.s32;
        }
        return first.data.s32 < second.data.s32;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT64:
        if (first.mask.u64 != second.mask.u64)
        {
            return first.mask.u64 < second.mask.u64;
        }
        return first.data.u64 < second.data.u64;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_MAC:
        if (compareMacAddress(first.mask.mac, second.mask.mac) != 0)
        {
            return compareMacAddress(first.mask.mac, second.mask.mac) < 0;
        }
        return compareMacAddress(first.data.mac, second.data.mac) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_IPV4:
        if (compareIpV4Address(first.mask.ip4, second.mask.ip4) != 0)
        {
            return compareIpV4Address(first.mask.ip4, second.mask.ip4) < 0;
        }
        return compareIpV4Address(first.data.ip4, second.data.ip4) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_IPV6:
        if (compareIpV6Address(first.mask.ip6, second.mask.ip6) != 0)
        {
            return compareIpV6Address(first.mask.ip6, second.mask.ip6) < 0;
        }
        return compareIpV6Address(first.data.ip6, second.data.ip6) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_OBJECT_ID:
        return first.data.oid < second.data.oid;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_OBJECT_LIST:
        return compareList(first.data.objlist, second.data.objlist) < 0;
    case SAI_ATTR_VALUE_TYPE_ACL_FIELD_DATA_UINT8_LIST:
        if (compareList(first.mask.u8list, second.mask.u8list) != 0)
        {
            return compareList(first.mask.u8list, second.mask.u8list) < 0;
        }
        return compareList(first.data.u8list, second.data.u8list) < 0;
    default:
        SWSS_LOG_THROW("Unhandled ACL field type %d", meta->attrvaluetype);
    }
