...
task_process_status BufferOrch::processEgressBufferProfileList(KeyOpFieldsValuesTuple &tuple)
{
    ...
    m_portBufferProfileListBulk.push_back(task);
...
void BufferOrch::processEgressBufferProfileListBulk(Consumer& consumer)
{
    ...
    for (const auto& task: m_portBufferProfileListBulk)
    {
        for (const auto& port: task.ports)
        {
            oids.push_back(port.port_oid);
    ...
    size_t i = 0;
    for (auto& task: m_portBufferProfileListBulk)
    {
        for (auto& port: task.ports)
        {
            port.status = statuses[i];
    ...
    m_portBufferProfileListBulk.clear();
}
...
void BufferOrch::doTask()
{
    ...
    if (WarmStart::isWarmStart())
    {
        ...
        const auto duration = std::chrono::duration_cast<second_t>(end - start).count();
        SWSS_LOG_NOTICE("Buffers configuration took %lf sec", duration);
    }
}
